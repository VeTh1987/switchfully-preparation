...
SELECT body, MAX(volume) FROM solar_system_objects;
SELECT body, AVG(volume) FROM solar_system_objects;


SELECT type_of_object, avg(volume) as avg_volume FROM solar_system_objects GROUP BY type_of_object HAVING avg_volume > 1;


SELECT COUNT(*),
       CASE
            WHEN volume  > 140000 THEN "Mega Big"
            WHEN volume  > 550 THEN "Super Big"
            WHEN volume  > 40 THEN "Big"
            ELSE "Still Quite Big"
        END as "big"
...
SELECT type_of_object,avg(volume) FROM solar_system_objects WHERE type_of_object like "%ice%" OR type_of_object like "%gas%" GROUP BY type_of_object;